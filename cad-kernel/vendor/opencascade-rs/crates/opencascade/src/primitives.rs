...
/// Iterator over the independent TopoDS_Solid sub-shapes of a Shape.
/// A boolean cut that physically severs a body into two disjoint pieces
/// produces a TopoDS_Compound containing two TopoDS_Solid children;
/// this iterator lets the backend body-tracker count and split them.
pub struct SolidIterator {
    pub(crate) explorer: UniquePtr<ffi::top_exp::TopExp_Explorer>,
}
...
impl Iterator for SolidIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.explorer.More() {
            let solid = ffi::topo_ds::TopoDS::Solid(self.explorer.Current());
