...
mod inner {
    unsafe extern "C++" {
        ...
        /// Asymmetric distance-distance chamfer. `dist1` is measured along
        /// the reference face `face`; `dist2` is along the opposite face.
        #[rust_name = "add_edge_two_distance"]
        pub fn Add(
        ...
        /// Distance + angle chamfer. `distance` is along the reference
        /// face `face`; `angle` (radians) is measured from that face.
        #[rust_name = "add_edge_distance_angle"]
        pub fn AddDA(
            self: Pin<&mut BRepFilletAPI_MakeChamfer>,
            distance: f64,
            angle: f64,
