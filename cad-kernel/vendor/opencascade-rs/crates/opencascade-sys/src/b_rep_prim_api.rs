...
mod inner {
    unsafe extern "C++" {
        ...
        // Both constructors call Init() eagerly, which calls Build()
        // internally. That can throw Standard_Failure (profile coincident
        // with axis, axis crossing the profile interior, etc.). We bind
        // via `try_construct_unique` (defined in bindings_common.hxx)
        // which wraps the call in a C++ try/catch and returns a NULL
        // UniquePtr on exception — Rust checks .is_null() to detect
        // failure. cxx's own Result<UniquePtr<...>> wrapping doesn't
        // reliably engage for template construct calls, so we do the
        // try/catch in C++ where it's certain to fire.
        #[cxx_name = "try_construct_unique"]
        ...
        /// 3-arg constructor: full closed revolution. Produces a CLOSED
        /// surface where the start and end positions of the source
        /// profile coincide as the SAME topology entity (the surface
        /// wraps parametrically) — no seam edges between distinct
        /// start/end caps. The 4-arg constructor above always treats
        /// the revolution as partial even at exactly 2π, leaving the
        /// source profile's edges visible at the seam.
        #[cxx_name = "try_construct_unique"]
        pub fn BRepPrimAPI_MakeRevol_full_new(
            shape: &TopoDS_Shape,
            axis: &gp_Ax1,
            copy: bool,
        ) -> UniquePtr<BRepPrimAPI_MakeRevol>;
