...
For a long stretch of the system's development, the precise mathematical solid of a mirrored instance was *not* reflected on export — the screen showed a correct mirror while the exported STEP file silently carried the unmirrored original. That gap is now closed: the full placement, including the reflection, is baked into the exact solid by the kernel at export time, and the same reflected geometry is used for interference checking. Mirrored, linear, and circular copies all export precisely.
...
**Exploded views** pull all parts apart to show how they fit together — the style of diagram used in assembly instructions. The editor provides a slider from zero (fully assembled) to one (fully exploded) that animates smoothly. On first use, an auto-explode derives a starting explosion by pushing each part radially outward from the assembly's geometric center. Crucially, the exploded view does not modify the assembly. Each part's display position receives an additional offset scaled by the slider value, applied at the last moment before drawing — entirely a presentation effect. The underlying mated positions are unchanged; sliding to zero restores the assembled view exactly. While the view is exploded, dragging components is disabled, so a user cannot accidentally re-position parts against a picture that does not reflect their real mated locations.
...
**Assembly export** produces a single 3D file — STEP for precise CAD interchange or STL for mesh exchange and 3D printing — with every part in its assembled position. The export applies each instance's placement transform — rotation, translation, and any mirror reflection — to its exact solid geometry and writes the results as one unified model. For released assemblies that have been frozen as official revisions, the export reads the stored frozen geometry directly rather than recomputing, guaranteeing byte-for-byte consistency with what was approved.
...
**The sharing goes all the way down to the database row.** A part's working design and an assembly's working design live in the *same* database table, distinguished by a single is-assembly flag; a part row carries the feature-tree document, an assembly row carries the component-and-mate document, and every version-control column between them is identical. There are not two parallel storage schemes to keep in sync — there is one, holding two document shapes.
...
- Mirror requires a winding-order fix to prevent surfaces rendering inside-out; the precise exported solid is also genuinely reflected — the kernel bakes rotation, translation, and reflection into the exact geometry for both export and interference checking.
