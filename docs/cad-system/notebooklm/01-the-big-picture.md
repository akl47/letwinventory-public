...
- The kernel is a **separate program** (written in native C++, built directly on the OCCT geometry library) that handles computationally intensive solid geometry operations. The modeler sends it a short instruction; it returns an exact mathematical shape plus a triangle mesh for rendering.
